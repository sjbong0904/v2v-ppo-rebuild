...
def choose_action(env: SumoIntersectionEnv, name: str) -> int:
    if name == "rule":
...
def run_policy(
    ...
    env = SumoIntersectionEnv(observation_mode=mode, pdr_scale=pdr_scale, seed=7)
    collisions = 0
    arrivals = 0
    near_misses = 0
    total_reward = 0.0
    ...
    try:
        for ep in range(episodes):
            ...
            episode_rows.append({
                ...
                "reward": ep_reward,
                "v2v_rx_count": int(info.get("v2v_rx_count", 0)),
                "aoi": float(info.get("aoi", 0.0)),
            })
    finally:
        env.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out-dir", default="runs/sumo_baseline")
    ...
    for mode in MODES:
        for policy in POLICIES:
            summary, rows = run_policy(policy, mode, args.episodes, args.pdr_scale)
            summaries.append(summary)
            details.extend(rows)
            print(
                f"{mode:12s} {policy:6s} "
                f"coll={summary['collision_rate']:.3f} "
                f"arr={summary['arrival_rate']:.3f} "
                f"near={summary['near_miss_rate']:.3f} "
                f"rew={summary['mean_reward']:.2f}"
            )
